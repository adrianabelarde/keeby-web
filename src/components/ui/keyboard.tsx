...
function KeyboardProvider({
  ...
  const loadSoundPromiseRef = useRef<Promise<void> | null>(null);
  ...
  useEffect(() => {
    if (!enableSound || !soundUrl) {
      ...
      loadSoundPromiseRef.current = null;
    }
  }, [enableSound, soundUrl]);

  const ensureSoundLoaded = useCallback(async () => {
    if (!enableSound || !soundUrl) return;
    if (audioBufferRef.current) return;
    if (loadSoundPromiseRef.current) return loadSoundPromiseRef.current;

    loadSoundPromiseRef.current = (async () => {
      try {
    ...
    return loadSoundPromiseRef.current;
  }, [enableSound, soundUrl]);
